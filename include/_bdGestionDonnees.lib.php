...
function connecterServeurBD() {
    $hote = "localhost";// le serveur sur lequel l'application fonctionne
    $login = "root";// le login pour se connecter à l'application
    $mdp = "root"; //Le mot de passe pour accèder à l'application
	$conn=new mysqli($hote,$login,$mdp);// Sert a faire la connexion à la base de donnéee
	
	if ($conn->connect_error){
		die('Erreur:'.$conn->connect_error);// si le mot de passe ou l'idenifiant ne correspond pas à la base de donnéee alors la connexion est un echec.
	}
}
...
function obtenirDetailVisiteur($unId) {
	...
	$db = new PDO('mysql:host=localhost;dbname=gsb_frais','root','root');
	$query = $db->prepare("select id, nom, prenom from visiteur where id='" . $unId . "'");
	$query->execute();
	$fetch = $query->fetch();
...
function obtenirDetailFicheFrais($idCnx, $unMois, $unIdVisiteur) {
	...
	$db = new PDO('mysql:host=localhost;dbname=gsb_frais','root','root');
	$query = $db->prepare("select IFNULL(nbJustificatifs,0) as nbJustificatifs, Etat.id as idEtat, libelle as libelleEtat, dateModif, montantValide 
...
function existeFicheFrais($idCnx, $unMois, $unIdVisiteur) {
	...
	$db = new PDO('mysql:host=localhost;dbname=gsb_frais','root','root');
	$query = $db->prepare("select idVisiteur from FicheFrais where idVisiteur='" . $unIdVisiteur . 
              "' and mois='" . $unMois . "'");
	$query->execute();
	$fetch = $query->fetch();
...
function obtenirDernierMoisSaisi($idCnx, $unIdVisiteur) {
	...
	$db = new PDO('mysql:host=localhost;dbname=gsb_frais','root','root');
	$query = $db->prepare("select max(mois) as dernierMois from FicheFrais where idVisiteur='" . $unIdVisiteur . "'");
	$query->execute();
	$fetch = $query->fetch();
...
function ajouterFicheFrais($idCnx, $unMois, $unIdVisiteur) {
	...
	$query = $db->prepare("insert into FicheFrais (idVisiteur, mois, nbJustificatifs, montantValide, idEtat, dateModif) values ('" 
              . $unIdVisiteur 
              . "','" . $unMois . "',0,NULL, 'CR', '" . date("Y-m-d") . "')");
	$query->execute();
    // ajout de la fiche de frais à l'état Cré
	/*
	...
	*/
	...
	$query = $db->prepare("select id from FraisForfait");
	$idJeuRes = $query->execute();
	/*
    ...
    if ( $idJeuRes ) {
       $fetch = $query->fetch();
        while ( is_array($fetch) ) {
            $idFraisForfait = $fetch["id"];
            // insertion d'une ligne frais forfait dans la base
			$query = $db->prepare("insert into LigneFraisForfait (idVisiteur, mois, idFraisForfait, quantite)
...
function verifierInfosConnexion($unLogin, $unMdp) {
	...
	$query = $db->prepare("select id, nom, prenom, login, mdp from Visiteur where login='".$unLogin."' and mdp='" . $unMdp . "'");
	$query->execute();
	$fetch = $query->fetch();
	return $fetch;
